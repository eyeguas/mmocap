...
*
*/
void StEv::setParams(BodyModel<HumanSkeleton> *body,ViewSet *vs) throw (gu::Exception)
{
  _nEvals=0;
    //it is required to be stereo information in the images passed
    for (unsigned int i=0;i<vs->size();i++) {
        if ( vs->get(i)->getDepth()==NULL) throw gu::Exception("StEv::setParams views must have stereo information");
//         if ( vs->get(i)->getStereoImage()->hasStereo()==false) throw gu::Exception("StEv::setParams stereo images must have stereo information calculated");
    }
    _TheViewSet=vs;
    _bodyModel=body;
    ...
    for (unsigned int v=0;v<_TheViewSet->size();v++) {
        ...
        if (mask->width!=mask0->width || mask->height!=mask0->height)
            throw gu::Exception("StEv::setParams images of different sizes not supported yet ");;
    }
