...
*
*/
void StXor::setParams(BodyModel<HumanSkeleton> *body,ViewSet *vs) throw (gu::Exception)
{
    _nEvals=0;
    //it is required to be stereo information in the images passed
    for (unsigned int i=0;i<vs->size();i++) {
        if ( vs->get(i)->getDepth()==NULL) throw gu::Exception("StXor::setParams views must have stereo information");
//         if ( vs->get(i)->getStereoImage()->hasStereo()==false) throw gu::Exception("StXor::setParams stereo images must have stereo information calculated");
    }
    _TheViewSet=vs;
    _bodyModel=body;
    ...
    for (unsigned int v=0;v<_TheViewSet->size();v++) {
        ...
        if (mask->width!=mask0->width || mask->height!=mask0->height)
            throw gu::Exception("StXor::setParams images of different sizes not supported yet ");;
    }
...
void ___writeVRML(string path,View *TV,BodyModel<HumanSkeleton> *bodyModel)
{
    ...
    file.close();
    cout<<"___writeVRML  Stereo info is not yet supported !!"<<__FILE__<<" "<<__LINE__<<endl;
...
double StXor::evaluate(BodyModel<HumanSkeleton> *body,ViewSet *vs) {
    ...
    for (unsigned int v=0;v<_TheViewSet->size();v++) {
        ...
        float *stXYZA=_TheViewSet->get(v)->getDepth();
                ...
                if (_invalid_v[index]==0 && visible[point]  ) { //the points has projected
                    ...
                    if (xo>=1 && xo+1<width && yo>=1 && yo+1<height) {
                        ...
                        if (maskpix[xo]) {
                            ...
                            if ( stXYZA[offSet*3+3]) {
                                nValid++;
                                mean+=stXYZA[offSet*3+2];
                            }
