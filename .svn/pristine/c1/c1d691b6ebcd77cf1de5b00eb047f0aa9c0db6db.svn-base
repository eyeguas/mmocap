...
*
*/
void XorColor::setParams(BodyModel<HumanSkeleton> *body,ViewSet *vs ) throw(GUException)
{
    _TheViewSet=vs;
    _bodyModel=body;
   _nEvals=0;
    ...
    for (unsigned int v=1;v<_TheViewSet->size();v++) {
        ...
        if (mask->width!=mask0->width || mask->height!=mask0->height)
            throw GUException("XorColor::setParams images of different sizes not supported yet");
    }
