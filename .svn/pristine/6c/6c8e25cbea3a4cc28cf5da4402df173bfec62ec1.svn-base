...
*
*/
void Surf::setParams(BodyModel<HumanSkeleton> *body,ViewSet *vs ) throw(gu::Exception) {
    _nEvals=0;
    _TheViewSet=vs;
    _bodyModel=body;
    ...
    for (unsigned int v=1;v<_TheViewSet->size();v++) {
        ...
        if (mask->width!=mask0->width || mask->height!=mask0->height)
            throw gu::Exception("Surf::setParams images of different sizes not supported yet");
    }
