...
*
*/
void OptFlow::setParams(BodyModel<HumanSkeleton> *body,ViewSet *vs ) throw(GUException) {
    _nEvals=0;
    _TheViewSet=vs;
    _bodyModel=body;
    ...
    for (unsigned int v=1;v<_TheViewSet->size();v++) {
        ...
        if (mask->width!=mask0->width || mask->height!=mask0->height)
            throw GUException("OptFlow::setParams images of different sizes not supported yet");
    }
