	...
	for (unsigned int a=0;a<_params.number_of_agents;a++)
		{
			...
			{
				_xx[a][b] = (double) gu::Random::nextFloat(_params.irang_l,_params.irang_r);
				//_xx[a][b] = (double) ((_params.irang_r - _params.irang_l)*(gu::Random::nextGaussian()) + _params.irang_l);
				_pbestx[a][b]=_xx[a][b];
				_vx[a][b] = _params.maxv*gu::Random::nextFloat(1.0);

				if ((gu::Random::nextFloat(1.0)) > 0.5) _vx[a][b]=-_vx[a][b];
	...
	for (unsigned int a=0;a<_params.number_of_agents;a++)
		{
			for (unsigned int b=0;b<_params.dimension;b++)
			{
				//_xx[a][b] = (double) gu::Random::nextFloat(x[b]-_params.irang_l,x[b]+_params.irang_r);
				_xx[a][b] = (double) ((_params.irang_l)*(gu::Random::nextGaussian()) + x[b]);
				_pbestx[a][b]=_xx[a][b];
				_vx[a][b] = _params.maxv*gu::Random::nextFloat(1.0);
 ...
 */ 
  void PSO::updateVelocityAndPosition(){
	...
	for (unsigned int b=0;b<_params.dimension;b++)
	{
		_vx[a][b] = _weight_up*_vx[a][b] + _params.c1*(gu::Random::nextFloat(1.0))*(_pbestx[a][b]-_xx[a][b]) +
					_params.c2*(gu::Random::nextFloat(1.0))*(_pbestx[_gbest][b]-_xx[a][b]);
