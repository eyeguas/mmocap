...
long   random_init_step(random_t *, long unsigned seed /* 0==clock */);
...
void readpara_init (readpara_t *, int dim, int seed,  const double * xstart, 
		    const double * sigma, int lambda, double stopMaxIter, double stopMaxEval, const char * filename);
void readpara_init_step (readpara_t *, int dim, int seed,  const double * xstart, 
...
void readpara_SupplementDefaults_step(readpara_t *);
...
double * 
cmaes_init_step(cmaes_t *t, /* "this" */
		int dimension, 
		double *inxstart,
		...
		const char *input_parameter_filename) 
{
  ...
  readpara_init_step (&t->sp, dimension, inseed, inxstart, inrgstddev, 
  		 lambda, input_parameter_filename);
  
  t->sp.seed = random_init_step( &t->rand, (unsigned) t->sp.seed);
...
/*
  ...
  t->rgxmean = new_double(N+2);*/ --t->rgxmean; t->rgxmean[0] = N; ++t->rgxmean;
  /*t->rgxold = new_double(N+2);*/ --t->rgxold; t->rgxold[0] = N; ++t->rgxold; 
  /*t->rgxbestever = new_double(N+3);*/ --t->rgxbestever;t->rgxbestever[0] = N; ++t->rgxbestever; 
  /*t->rgout = new_double(N+2);*/ --t->rgout; t->rgout[0] = N; ++t->rgout;
  //t->rgD = new_double(N);
...
cmaes_init(cmaes_t *t, /* "this" */
		...
		double *inxstart,
	        double *inrgstddev, /* initial stds */
		long int inseed,
		int lambda, double stopMaxIter, double stopMaxEval,
		const char *input_parameter_filename) 
{
  int i, j, N;
  double dtest, trace;
  ...
  readpara_init (&t->sp, dimension, inseed, inxstart, inrgstddev, 
  		 lambda, stopMaxIter, stopMaxEval, input_parameter_filename);
...
//		      printf( "TolFun: function value differences %7.2e < stopTolFun=%7.2e\n", 
//		      range, t->sp.stopTolFun);
      }
...
//	printf( "TolFunHist: history of function value changes %7.2e stopTolFunHist=%7.2e", 
//			range, t->sp.stopTolFunHist);
	...
	cp += sprintf(cp, 
		      ...
		      t->sp.stopTolX);
//	printf("TolX: object variable changes below %7.2e \n", 
//		      t->sp.stopTolX);
      }
...
//	printf("TolUpX: standard deviation increased by more than %7.2e, larger initial standard deviation recommended \n", 
//		      t->sp.stopTolUpXFactor);
...
//	printf( "ConditionNumber: maximal condition number %7.2e reached. maxEW=%7.2e,minEW=%7.2e,maxdiagC=%7.2e,mindiagC=%7.2e\n", 
//		      t->dMaxSignifKond, t->maxEW, t->minEW, t->maxdiagC, t->mindiagC);
      } /* if */
	  ...
	  if (iKoo == N)	
	    {
	      ...
	      cp += sprintf(cp, 
			    ...
			    fac/0.1, iAchse);
//	     printf("NoEffectAxis: standard deviation 0.1*%7.2e in principal axis %d without effect\n", 
//			    fac/0.1, iAchse);
...
//	      printf("NoEffectCoordinate: standard deviation 0.2*%7.2e in coordinate %d without effect\n", 
//			    t->sigma*sqrt(t->C[iKoo][iKoo]), iKoo); 
...
//	printf("MaxFunEvals: conducted function evaluations %.0f >= %g\n", 
//		      t->countevals, t->sp.stopMaxFunEvals);
...
//	printf("MaxIter: number of iterations %.0f >= %g\n", 
//		      t->gen, t->sp.stopMaxIter);
      }
      if(t->flgStop){
	cp += sprintf(cp, "Manual: stop signal read\n");
//	printf("Manual: stop signal read\n");
      }
#if 0
  else if (0) {
    for(i=0, cTemp=0; i<N; ++i) {
...
long 
random_init_step( random_t *t, long unsigned inseed)
{
  clock_t cloc = clock();
...
/* --------------------------------------------------------- */
void
readpara_init_step (readpara_t *t,
	       int dim, 
	       int inseed, 
...
*/
  ...
  readpara_SupplementDefaults_step(t);
} /* readpara_init */
	       ...
	       const char * filename)
{
  ...
  */
  ...
  t->rgInitialStds = NULL; 
  t->rgDiffMinChange = NULL; 
  t->stopMaxFunEvals = stopMaxEval;
  t->stopMaxIter = stopMaxIter;
  t->facmaxeval = 1; 
  t->stStopFitness.flg = 1;
  t->stStopFitness.val = 0;
  t->stopTolFun = 1e-12; 
  t->stopTolFunHist = 1e-13; 
  t->stopTolX = 1e-11; /* 1e-11*insigma would also be reasonable */ 
  t->stopTolUpXFactor = 1e5; 
...
/* --------------------------------------------------------- */
void 
readpara_SupplementDefaults_step(readpara_t *t)
{
  double t1, t2;
  int N = t->N; 
  clock_t cloc = clock();
