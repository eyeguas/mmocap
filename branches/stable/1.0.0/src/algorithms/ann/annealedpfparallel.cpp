...
/**
*/
void AnnealedPFParallel::setParams(Particle initParticle, vector<double> noise,Parameters  P)throw(GUException)
{
    if (noise.size()!=initParticle.size())
        throw GUException("AnnealedPFParallel::setParams noise has not the same dimensions than initParticle");
    _noise=noise;
...
*
*/
void AnnealedPFParallel::start()   throw(GUException)
{
    if (!_areParms) throw  GUException("AnnealedPFParallel::start no call to setParams()");
    if (_currentLayer!=-1 && _currentLayer<_params.nLayers) throw  GUException("AnnealedPFParallel::start end of layers not reached last time");
    _currentLayer=0;
}
...
*
*/
void AnnealedPFParallel::nextStep()throw(GUException)
{
    if (!_areParms) throw  GUException("AnnealedPFParallel::nextStep no call to setParams()");
    if (_currentLayer>=_params.nLayers) throw  GUException("AnnealedPFParallel::nextStep  number of iterations exceeded");
    if (_currentLayer==-1) throw  GUException("AnnealedPFParallel::nextStep  no call to start");
...
*
*/
bool AnnealedPFParallel::end()throw(GUException)
{
    if (!_areParms) throw  GUException("AnnealedPFParallel::nextStep no call to setParams()");
...
//     double invN=1./float(mean.size());
    ...
    if (_currentLayer==0)//first time, user defined noise
    {
        for (unsigned int i=0;i<currParticles->size();i++)
        {
		  ...
		  (*p)[j]+=GURandom::nextGaussian() * _noise[j];
	    }
        }
    }
...
/* for (unsigned int i=0;i<(*currParticles)[0] ->size();i++)
 ...
 cout<<endl;*/
        for (unsigned int i=0;i<currParticles->size();i++)
        {
            ...
            for (unsigned int j=0;j<p->size();j++){ 
	      if (_activeElements[j])//variate only if allowed by the mask
                (*p)[j]+=GURandom::nextGaussian() * var[j];
	    }
        }
    }
}
...
//
/////////////////////////////////
void AnnealedPFParallel::setMask(vector<bool> activeElements)throw(GUException)
{
if (  activeElements.size()!=_activeElements.size()) throw GUException("AnnealedPFParallel::setMask the mask must have the same size than variable in the search space");
