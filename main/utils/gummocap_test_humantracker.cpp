...
#ifdef HAS_STEREO_SUPPORT
#include "evaluators/stev.h"
#include "evaluators/stxor.h"
#include "evaluators/stxorcolor.h"
#endif
#ifdef HAS_GPU_SUPPORT
#include "gpu/gpu_xor.h"
#endif
#include "evaluators/countforeg.h"
#include "evaluators/ramanan.h"
...
///Video sequence to directory of  images
string TheVideoSeqDir;
bool saveVideoSeq2Dir=false;
bool saveVideoSeq2DirInit=false;
...
double PSOInitialWeight=2.0; // 2.0
////////// DE ////////////////
...
vector<bool> *_DOFXYZMask=NULL; //mask for limiting the number of DOFs in the joints
...
vector<bool> * createReducedXYZModelMask();
///Error Analisys
...
void usage()
{
    ...
    cout<<" --videoSeq2Dir <dir>: saves the video sequence of images to a directory"<<endl;
    ...
    cout<<" -f fitness_function (ramanan,xor,fog,fogvh,st,stxor,stxorcolor,xorcolor,xorgrad,glxor,surf,optflow,gpu_xor)"<<endl;
    ...
    cout<<" --reduceModel: uses a reduced human model with less DOF (joints)"<<endl;
    cout<<" --reduceModelXYZ: uses a reduced human model with less DOF (X,Y,Z)"<<endl;
            ...
            long_options [] =
{
    ...
    { "reduceModelXYZ",     no_argument,   NULL,           326 },
    ...
    {"videoSeq2Dir", required_argument,   NULL,           604 },
    { 0, 0, 0, 0 }
};
...
void readArguments ( int argc,char **argv )
{
    for ( ;; )
    {
        ...
        case 313:
            ...
            break;
	case 326:
            _DOFXYZMask=createReducedXYZModelMask();
            break;
        case 314:
            HS=true;
	...
	case 603:
	    ...
	    break;
	case 604:
	      TheVideoSeqDir=optarg;
	      saveVideoSeq2Dir=true;	  
	  break;
        default:
            usage ();
            exit ( EXIT_FAILURE );
...
//
/////////////////////////////////////////////
gummocap::poseevaluators::PoseEvaluator* getIndividualFitnessEvaluator ( string type ) throw ( gu::Exception )
{
cout<<"KKJKK "<<type<<endl;
 ...
 if ( type=="ramanan" )
    {
        gummocap::poseevaluators::Ramanan *FF=new Ramanan();
        FF->setParams ( &ThePersonModel,TheViewSet );
        return FF;
    }
    else if ( type=="fog" )
    ...
    #endif
#ifdef HAS_GPU_SUPPORT
    else if ( type=="gpu_xor" )
    {
        gummocap::poseevaluators::Gpu_Xor *FF=new gummocap::poseevaluators::Gpu_Xor();
        FF->setParams ( &ThePersonModel,TheViewSet );
        return FF;
    }
    ...
    {
        throw gu::Exception ( "getFitnessEvaluator invalid evaluator descriptor :"+type );
    }
}
...
//
/////////////////////////////////////////////
gummocap::poseevaluators::PoseEvaluator* getFitnessEvaluator ( string type ) throw ( gu::Exception )
{
//detect if several evaluator are indicated
    ...
    {
        if ( foundPLus&foundMult ) throw gu::Exception ( "Error combining evaluators use only + or *" );
...
//
/////////////////////////////////////////////
gummocap::algorithms::HumanTracker* getHumanTracker ( string type,BodyModel<HumanSkeleton> &bodyModel,ViewSet *VS,gummocap::poseevaluators::PoseEvaluator* posEv ) throw ( gu::Exception )
{
    ...
    else if ( type=="pso" )
    {
	    ...
	    if(_DOFXYZMask==NULL)
	      ...
	      Alg->setParams ( bodyModel, VS,posEv, PSOHumanPartTracker::Params ( PSONoOfAgents,PSOMaximumVelocity,PSOSearchRadius,PSOInitialWeight,NumFunctionFitnessEvaluations,nJointsBodyandHead,nJointsLeftArm,nJointsRightArm,nJointsLeftLeg,nJointsRightLeg ),_DOFMask,_DOFXYZMask,numHsteps );
            return Alg;
        }
        else
        {
	    ...
	    if(_DOFXYZMask==NULL)
	      ...
	      Alg->setParams ( bodyModel, VS,posEv, PSOHumanTracker::Params ( PSONoOfAgents,PSOMaximumVelocity,PSOSearchRadius,PSOInitialWeight,NumFunctionFitnessEvaluations ),_DOFMask,_DOFXYZMask );
	    return Alg;
        }
    }
    else if ( type=="de" )
    {
	    ...
	    if(_DOFXYZMask==NULL)
	      ...
	      Alg->setParams ( bodyModel, VS,posEv, DEHumanPartTracker::Params ( nJointsBodyandHead,nJointsLeftArm,nJointsRightArm,nJointsLeftLeg,nJointsRightLeg,DEPopulationSize,DEnLPC,DESearchRadius,DESearchRadius,DE_CR,DE_F,NumFunctionFitnessEvaluations ),_DOFMask,_DOFXYZMask,numHsteps );
	    ...
	    if(_DOFXYZMask==NULL)
	      ...
	      Alg->setParams ( bodyModel, VS,posEv, DEHumanTracker::Params ( DEPopulationSize,DEnLPC,DESearchRadius,DESearchRadius,DE_CR,DE_F,NumFunctionFitnessEvaluations ),_DOFMask,_DOFXYZMask );
            return Alg;
        }
    }
    else if ( type=="psapf" )
    ...
    {
        throw gu::Exception ( "getHumanTracker invalid algorithm descriptor :"+type );
    }
    return NULL;
}
...
//
/////////////////////////////////////////////
ViewSetProducer * getViewSetProducer() throw ( gu::Exception )
{
    switch ( inputType )
    {
    case 0:
...
//          return TheVSReader;
    ...
    default:
        throw gu::Exception ( "getViewSetProducer() invalid input type" );
...
int main ( int argc,char **argv )
{
    try
    {
//         if ((argc<4)||(argc>5)) throw gu::Exception("Usage: human_bodymodel.hbm  video.vis background.bck [out.avi]");
        readArguments ( argc,argv );
        if ( TheInputVideoFile=="" ) throw gu::Exception ( "input required" );
        if ( TheInputModelFile=="" ) throw gu::Exception ( "-m required" );
        if ( TheFitnessFunction=="" ) throw gu::Exception ( "-f required" );
        ...
        for ( unsigned int i=0;i<TheViewSet->size();i++ )
        {
            if ( TheViewSet->get ( i )->getCameraParameters()->Extrinsics.isValid() ==false )
                throw gu::Exception ( "Invalid extrinsics parameters in any of the streams given" );
        }
        if ( ( FF=getFitnessEvaluator ( TheFitnessFunction ) ) ==NULL )
            throw gu::Exception ( "Invalid fitness function" );
        ...
        if ( PoseEstimation!=-1 )
        {
            ...
            if ( !TheDoAnalyzeError ) throw gu::Exception ( "In PoseEstimation mode, the --errorFile option must be provided" );
            ...
            if ( TheViewSetReader->eof() ) throw gu::Exception ( "The frame indicated for pose estimation is not correct. The video is not that long" );
            ...
            if ( MS==NULL ) throw gu::Exception ( "There is not groundtruth info for the frame indicated" );
            if ( (unsigned int)MS->getNValidMarkers() !=MS->size() ) throw gu::Exception ( "Groundtruth for the frame indicated is not valid" );
            //
...
//the model are based on ImageSets and not ViewSets
		...
		for(unsigned int i=0;i<validViews.size();i++) 
 		...
 		cout<<"<conf> frame: "<<TheViewSetReader->currentFrameIndex() <<", nviews : "<< nValidViews<<" </conf>"<<endl;
		   ...
		   NewVideo.open(TheOutAviFile , CV_FOURCC('D','I','V','X'), 30, cv::Size(composed->getWidth(),composed->getHeight()), 10);
		   if (! NewVideo.isOpened() ){cerr<<"Could not open video"<<endl;exit(0);}
                }
               
                NewVideo<< guimage::opencv::getMat( composed  );
            }
            if (saveVideoSeq2Dir){
		if (saveVideoSeq2DirInit==false){
		    string cmd="mkdir "+TheVideoSeqDir+" -p";
		    system(cmd.c_str());
		    cmd="rm "+TheVideoSeqDir+"/* -rf";
		     system(cmd.c_str());
		    saveVideoSeq2DirInit=true;
		}
		string name=gu::StringUtils::composedFileNameWithNumber (TheVideoSeqDir+"/frame_v-",TheViewSetReader->currentFrameIndex()  , 3, ".png");
		 
// 		cv::Mat im=guimage::opencv::getMat(composed);
		cv::Mat im=guimage::opencv::getMat(TheISCopy.get(5));		
		cv::imwrite(name,im);	      
	    }
            //analyze error
            if ( TheDoAnalyzeError )
...
vector<bool> * createReducedXYZModelMask()
{
    ...
    for ( unsigned int i=0;i<mask->size();i++ ) ( *mask ) [i]=true;
    //
    ( *mask ) [HumanSkeleton::Lelbow_X]=false;
    ( *mask ) [HumanSkeleton::Lelbow_Z]=false;
    ( *mask ) [HumanSkeleton::Relbow_X]=false;
    ( *mask ) [HumanSkeleton::Relbow_Z]=false;
    ( *mask ) [HumanSkeleton::Lwrist_X]=false;
    ( *mask ) [HumanSkeleton::Rwrist_X]=false;
    
    ( *mask ) [HumanSkeleton::Lknee_Y]=false;
    ( *mask ) [HumanSkeleton::Lknee_Z]=false;
    ( *mask ) [HumanSkeleton::Rknee_Y]=false;
    ( *mask ) [HumanSkeleton::Rknee_Z]=false;
    ( *mask ) [HumanSkeleton::Lankle_Z]=false;
    ( *mask ) [HumanSkeleton::Rankle_Z]=false;
