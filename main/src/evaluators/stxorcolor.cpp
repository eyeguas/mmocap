...
*
*/
void StXorColor::setParams(BodyModel<HumanSkeleton> *body,ViewSet *vs) throw (gu::Exception)
{
  
    //it is required to be stereo information in the images passed
    for (unsigned int i=0;i<vs->size();i++) {
        if ( vs->get(i)->getDepth()==NULL) throw gu::Exception("StXorColor::setParams views must have stereo information");
//         if ( vs->get(i)->getStereoImage()->hasStereo()==false) throw gu::Exception("StXorColor::setParams stereo images must have stereo information calculated");
    }
    _TheViewSet=vs;
    _bodyModel=body;
   _nEvals=0;
    ...
    for (unsigned int v=0;v<_TheViewSet->size();v++) {
        ...
        if (mask->width!=mask0->width || mask->height!=mask0->height)
            throw gu::Exception("StXorColor::setParams images of different sizes not supported yet ");;
    }
            ...
            for (unsigned point=0;point<vertices.size() ;point++,index+=2) {
                if (_invalid_v[index]==0 && visible[point]  ) { //the points has projected
                    ...
                    if (xo>=1 && xo+1<bgr->width && yo>=1 && yo+1<bgr->height) {
                        ...
                        if (maskpix[xo] &&  stXYZA[offSet*3+3]) {
                            //now, calculate the dsitance to the expected point
                            float dist=stXYZA[offSet*3+2]-_zvertices[point];
                            if (dist<0) dist=-dist;
