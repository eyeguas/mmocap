...
*
*/
void FogVH::setParams(BodyModel<HumanSkeleton> *bodyModel,ViewSet *TheViewSet ) throw(gu::Exception)
{
    _nEvals=0;
    _areParams=true;
    ...
    for(unsigned int i=0;i<TheViewSet->size();i++)
      if(TheViewSet->get(i)->getMaskIntegral()==NULL) throw gu::Exception("FogVH::setParams integral masks must be calculated");
