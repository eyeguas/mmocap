...
/**Sets the required params and initializes
*/
void DEHumanPartTracker::setParams(BodyModel<HumanSkeleton> &sk,guscene::view::ViewSet *VSet, PoseEvaluator* fitness_func, Params  p, vector<bool> *DOFmask, vector<bool> *DOFXYZmask,unsigned int nBodyParts)
{
   ...
   //gumocap::skeleton::SkeletonPoseWoT::nDOF(*sk.getSkptr(), DOFmask, DOFXYZmask);
	...
	_pffs.push_back(vector<FitnessFunction* >());
	for(int i=0;i<omp_get_max_threads();i++){
	  _pffs[h].push_back(new HPTFitnessFunction(_xbest,_N,_ini_end_body_parts[h*2],_ini_end_body_parts[h*2+1],_evaluators[i],_auxBodyModelV[i],_TheViewSet,DOFmask,_reducedDOFXYZmask));
	}
    }
...
/*
*/
void DEHumanPartTracker::step(guscene::view::ViewSet *VSet)throw(gu::Exception)
{
  _TheViewSet=VSet;
