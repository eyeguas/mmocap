...
void CondensationHumanTracker::step(guscene::view::ViewSet *VSet)throw(gu::Exception)
{
...
void CondensationHumanTracker::variateParticle(Particle &src,Particle &dst)
{
    ...
    dst[0]=src[0]+ gu::Random::nextGaussian()*0.01;
    dst[1]=src[1]+ gu::Random::nextGaussian()*0.01;
    dst[2]=src[2]+ gu::Random::nextGaussian()*0.01;
    float devGrad=(1.*M_PI)/180.;
    //variate only the shoulder and elbow

    for (unsigned int i=3;i<src.size();i++)
        dst[i]=src[i]+ gu::Random::nextGaussian()*devGrad; 
}
/**
*
*/
